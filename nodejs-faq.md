...
```js
/**
 * read_file.js (async/await with fs.promises — recommended for Node.js 10+)
 */
...
// Built-in parsers (Express 4.16+ — no separate body-parser needed)
...
> **Note (Express 4.16+):** `express.json()` and `express.urlencoded()` are now built into Express, so a separate `body-parser` package is no longer needed for most use cases.

**Example (Express built-in parsers — recommended):**

```js
const express = require("express");
const app = express();

// Built-in JSON parser (replaces bodyParser.json())
app.use(express.json());

// Built-in URL-encoded parser (replaces bodyParser.urlencoded())
app.use(express.urlencoded({ extended: false }));
...
**Legacy usage (still works, but not required for Express 4.16+):**
...
> **Note:** Moment.js is in maintenance-only mode and no longer receives new features. For new projects prefer **[Day.js](https://day.js.org/)** (2 KB, same API) or **[Luxon](https://moment.github.io/luxon/)** (full-featured, immutable). The examples below use Day.js.
...
dayjs().format('MMMM DD YYYY, h:mm:ss a'); // April 17 2026, 3:15:22 pm
dayjs().format('dddd');                     // Friday
dayjs().format('MMM DD YY');                // Apr 17 26
```
...
dayjs().startOf('day').fromNow(); // 15 hours ago
```

* Calendar Time (requires `calendar` plugin)
...
dayjs().subtract(10, 'day').calendar(); // 04/07/2026
dayjs().subtract(6, 'day').calendar();  // Last Sunday at 3:18 PM
dayjs().subtract(3, 'day').calendar();  // Last Wednesday at 3:18 PM
```
...
Node.js 18+ ships with a native `fetch` API, so no extra dependency is needed.

```js
/**
 * POST Request using native fetch (Node.js 18+)
 */
...
**Making HTTP Requests (native fetch – Node.js 18+):**

```js
// Simple GET
const response = await fetch('https://nodejs.org/');
...
```js
// GET with options
const response = await fetch('https://nodejs.org/file.json', {
    method: 'GET',
    headers: {
...
The `fetch` API is available globally in Node.js 18+ without any `require`/`import`. For environments older than Node.js 18, use the `node-fetch` or `axios` package.
...
* Encryption example using Cipher (`createCipheriv` – required since Node.js 22)

```js
const crypto = require('crypto');

const algorithm = 'aes-192-cbc';
const key = crypto.scryptSync('a password', 'salt', 24); // derive 192-bit key
const iv = crypto.randomBytes(16);                        // random initialisation vector
...
console.log('IV (store alongside ciphertext):', iv.toString('hex'));
```

* Decryption example using Decipher (`createDecipheriv` – required since Node.js 22)
...
// iv and encryptedText must match values produced during encryption
const iv = Buffer.from('<hex-iv-from-encryption>', 'hex');
const encryptedText = '<hex-ciphertext-from-encryption>';
...
Node.js runs in a single thread by default, which means it cannot take full advantage of multi-core CPUs. The built-in `cluster` module solves this by allowing you to spawn multiple worker processes that all share the same server port. The primary (master) process manages the workers; each worker runs its own event loop.

**How it works:**

* The primary process calls `cluster.fork()` to create worker processes.
* Workers are full Node.js processes that can accept incoming connections.
* The OS distributes incoming connections across workers (round-robin on most platforms).
* If a worker crashes, the primary process can fork a replacement.

**Example 01:** Basic Cluster Setup

```js
/**
 * Node.js Cluster - Basic Setup
 */
...
**Example 02:** Graceful Shutdown of a Worker
...
if (cluster.isPrimary) {
  ...
  // Send a shutdown signal after 5 seconds
  setTimeout(() => {
    worker.send('shutdown');
    worker.disconnect();

    // Force kill if the worker does not exit within 3 seconds
    setTimeout(() => {
      if (!worker.isDead()) {
        worker.kill();
      }
...
**Example 03:** Communication Between Primary and Workers
...
if (cluster.isPrimary) {
  ...
  process.on('message', (msg) => {
    ...
    process.send({ type: 'result', workerId: process.pid, status: 'done' });
  });
}
```

**Example 04:** Managing Clusters with PM2

PM2 is the recommended way to manage clusters in production. It handles forking, monitoring, log aggregation, and zero-downtime reloads automatically.

```bash
# Install PM2 globally
npm install pm2 -g

# Start app in cluster mode (one instance per CPU core)
...
module.exports = {
  apps: [{
    name: 'my-app',
    script: 'app.js',
    instances: 'max',   // one per CPU core; use a number for a fixed count
    exec_mode: 'cluster',
    watch: false,
    env: {
      NODE_ENV: 'production'
...
**Key `cluster` module events and methods:**

| Method / Event | Description |
|---|---|
| `cluster.fork()` | Spawn a new worker process |
| `cluster.isPrimary` | `true` in the primary process |
| `cluster.isWorker` | `true` in a worker process |
| `cluster.workers` | Object containing all live worker references |
| `worker.send(msg)` | Send a message to a worker from the primary |
| `process.send(msg)` | Send a message to the primary from a worker |
| `cluster.on('exit')` | Fired when a worker dies |
| `cluster.on('online')` | Fired when a worker comes online |
| `worker.disconnect()` | Gracefully stop accepting connections |
| `worker.kill()` | Forcefully terminate a worker |
